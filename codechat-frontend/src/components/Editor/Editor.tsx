...
const Editor = () => {
    const socketConnection = useSelector(
        (state: ReturnType<typeof RootReducer>) => state.socket.instance
    )
    const editorCode = useSelector(
        (state: ReturnType<typeof RootReducer>) => state.room.editorCode
    )
    const roomCode = useSelector(
        (state: ReturnType<typeof RootReducer>) => state.room.roomCode
    )
    ...
    useEffect(() => {
        socketConnection?.on(
            SocketActionType.CODE_CHANGE,
            ({ code }: { [key in string]: string }) => {
                console.log(code)
                setCode(code)
            }
        )
    ...
    return (
        <div className="overflow-hidden min-w-[100%] min-h-[100%] ">
            <CodeMirror
                className="text-black  min-h-full min-w-full"
                value={code}
                onChange={(code) => {
                    handleChanges(code)
                }}
